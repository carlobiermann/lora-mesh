// Testing sending and receiving various LoRa Packets 
// and invalid/valid LDM Frames, as well as forging 
// an ACK handshake. 

// This code is intended to be tested with 2 T-Beams: 
// one Transmitter Node and one Receiver Node. 
// The only difference between those two in the 
// code is that Transmitter Node will run in Parent Mode 
// and Receiver Node will run in Node Mode. 
// There are also 7 lines inside the loop() that should 
// be commented out when the code is intended for the Receiver
// Node.
...
// GLOBAL VARIABLES
int i = 18;                          // for ldm_frameTester(int i)
// int mode = 0;                    // Node Mode
int mode = 1;                       // Parent Mode
byte routingTable[153] = "";
byte payload[24] = "";
byte localAddress = 0xAA;        // Transmitter Node
// byte localAddress = 0x22;    // Receiver Node
...
void loop(){
  int result = ldm_listener(LoRa.parsePacket(), mode);
  
  if(result != 0){
    Serial.println(result);
    //ldm_printRoutingTable();
  ...
  // to there when used as Receiver Node
}

void ldm_frameTester(int i){

// Testing various LoRa Packets and 
// invalid/valid LDM Frames as well as the ACK 
// handshake
  ...
  header[1] = 0x42;           // type
  header[2] = 0x22;           // router: Receiver Node ID 
  header[3] = 0x55;           // source
  header[4] = 0x22;           // recipient: Receiver Node ID
  header[5] = 0x11;           // sender
  header[6] = 0x0E;           // ttl
  ...
  switch(i){
    ...
    // Sending a LoRa Packet with frameSize 9
  // Receiver - Expect: 0
    case 0: 
      LoRa.beginPacket();
    for(int j=0; j<9; j++){
    ...
    // Sending a LoRa Packet with frameSize 33
  // Receiver - Expect: 0
    case 1: 
      LoRa.beginPacket();
    for(int j=0; j<33; j++){
    ...
    // Sending a LoRa Packet with frameSize 7  
  // Receiver - Expect: 0
    case 2: 
      LoRa.beginPacket();
    for(int j=0; j<7; j++){
    ...
    // Sending a LoRa Packet with frameSize 31
  // Receiver - Expect: 0
    case 3: 
      LoRa.beginPacket();
       for(int j=0; j<31; j++){
    ...
    // Sending valid LDM Header with invalid sizeHeader
  // Receiver - Expect: 0
    case 4:
      header[0] = 0x05;     // invalid sizeHeader
      LoRa.beginPacket();
      LoRa.write(header, 8);
      LoRa.endPacket(true);  
      Serial.println(i);  
    ...
    // Sending valid LDM Header with invalid type
  // Receiver - Expect: 0
    case 5: 
      header[1] = 0x54;     // invalid type
      LoRa.beginPacket();
      LoRa.write(header, 8);
      LoRa.endPacket(true);   
      Serial.println(i); 
    ...
    // Sending valid LDM Header with invalid router
  // Receiver - Expect: 0
    case 6: 
      header[2] = 0x88;     // invalid router
      LoRa.beginPacket();
      LoRa.write(header, 8);
      LoRa.endPacket(true);  
      Serial.println(i);  
    ...
    // Sending valid LDM Header with invalid source
  // Receiver - Expect: 0
    case 7: 
      header[3] = 0x67;     // invalid source
      LoRa.beginPacket();
      LoRa.write(header, 8);
      LoRa.endPacket(true);
      Serial.println(i);     
    ...
    // Sending valid LDM Header with invalid recipient
    // Receiver - Expect: 0
    case 8: 
      header[4] = 0x89;     // invalid recipient
      LoRa.beginPacket();
      LoRa.write(header, 8);
      LoRa.endPacket(true);  
      Serial.println(i);   
    ...
    // Sending valid LDM Header with invalid sender
    // Receiver - Expect: 0
    case 9: 
    header[5] = 0x43;     // invalid sender
      LoRa.beginPacket();
      LoRa.write(header, 8);
      LoRa.endPacket(true); 
      Serial.println(i);    
    ...
    // Sending valid LDM Header with invalid ttl (too large)
  // Receiver - Expect: 0
    case 10: 
    header[6] = 0x10;     // invalid ttl
      LoRa.beginPacket();
      LoRa.write(header, 8);
      LoRa.endPacket(true);  
      Serial.println(i);   
    ...
    // Sending valid LDM Header with invalid ttl (0x00)
  // Receiver - Expect: 0
    case 11: 
    header[6] = 0x00;     // invalid ttl
      LoRa.beginPacket();
    LoRa.write(header, 8);
      LoRa.endPacket(true); 
      Serial.println(i);    
    ...
    // Sending valid LDM Header with invalid sizePayload 
  // Receiver - Expect: 0
    case 12: 
    header[7] = 0x01;     // invalid sizePayload
      LoRa.beginPacket();
    LoRa.write(header, 8);
      LoRa.endPacket(true);
      Serial.println(i);     
  ...
  // Receiver - Expect: 0
    case 13: 
    header[7] = 0x19;     // invalid sizePayload
      LoRa.beginPacket();
    LoRa.write(header, 8);
      LoRa.endPacket(true); 
      Serial.println(i);    
    ...
    // Sending valid LDM Header with of Type C (0x43) 
    // Not relevant for Node Mode 
  // Receiver - Expect: 0
    case 14:
    header[1] = 0x43;     // Type C
      LoRa.beginPacket();
    LoRa.write(header, 8);
      LoRa.endPacket(true); 
      Serial.println(i);     
    ...
    // Sending valid LDM Header with of Type E (0x45) 
    // Not relevant for Node Mode
  // Receiver - Expect: 0
    case 15:
    header[1] = 0x45;     // Type E
      LoRa.beginPacket();
    LoRa.write(header, 8);
      LoRa.endPacket(true); 
      Serial.println(i);     
    ...
    // Sending valid LDM Header but not addressed at Receiver Node
    case 16: 
      header[2] = 0x33;     // router
      LoRa.beginPacket();
    LoRa.write(header, 8);
      LoRa.endPacket(true);  
      Serial.println(i);   
    ...
    // Sending valid LDM ACK Frame but not addressed at Receiver Node
    // Not relevant for Node Mode
  // Receiver - Expect: 0
    case 17: 
    header[1] = 0x45;     // Type E 
    header[2] = 0x44;     // router
      LoRa.beginPacket();
    LoRa.write(header, 8);
      LoRa.endPacket(true);  
      Serial.println(i);  
    ...
    // Sending a valid Type B BCAST to fill Receiver's Routing Table 
    // and Routing Status
    // Entry 1
    // Receiver - Expect: 1
    //     localHopCount: 0x05
    ...
    case 18: 
      header[0] = 0x08;           // sizeHeader
      header[1] = 0x42;           // type
      header[2] = 0xFF;           // router
      header[3] = 0x55;           // source: Node 5
      header[4] = 0xFF;           // recipient
      header[5] = 0x55;           // sender: Node 5
      header[6] = 0x0E;           // ttl
      header[7] = 0x02;           // sizePayload
      
    LoRa.beginPacket();
    LoRa.write(header, 8);
    LoRa.write(0x04);       // payload: localHopCount
    LoRa.write(0x66);           // payload: localNextHopID 
      LoRa.endPacket(true);   
      Serial.println(i);  
      break;  
  ...
  //    localNextHopID: 0x77
    ...
    header[0] = 0x08;           // sizeHeader
      header[1] = 0x42;           // type
      header[2] = 0xFF;           // router
      header[3] = 0x77;           // source: Node 7
      header[4] = 0xFF;           // recipient
      header[5] = 0x77;           // sender: Node 7
      header[6] = 0x0E;           // ttl
      header[7] = 0x02;           // sizePayload
      
    LoRa.beginPacket();
    LoRa.write(header, 8);
    LoRa.write(0x03);       // payload: localHopCount
    LoRa.write(0x33);           // payload: localNextHopID 
      LoRa.endPacket(true);   
      Serial.println(i);  
      break; 

    // Entry 3 - but invalid localNextHopID
  // Receiver - Expect: 1 
  //     localHopCount: 0x04
  //    localNextHopID: 0x77
    case 20: 
    header[0] = 0x08;           // sizeHeader
      header[1] = 0x42;           // type
      header[2] = 0xFF;           // router
      header[3] = 0x44;           // source: Node 4
      header[4] = 0xFF;           // recipient
      header[5] = 0x44;           // sender: Node 4
      header[6] = 0x0E;           // ttl
      header[7] = 0x02;           // sizePayload
      
    LoRa.beginPacket();
    LoRa.write(header, 8);
    LoRa.write(0x02);       // payload: localHopCount
    LoRa.write(0x22);           // payload: localNextHopID 
      LoRa.endPacket(true); 
      Serial.println(i);    
      break;     

    // Entry 4 PARENT BCAST
  // Receiver - Expect: 1 
  //     localHopCount: 0x01
  //    localNextHopID: 0xAA
    case 21: 
    header[0] = 0x08;           // sizeHeader
      header[1] = 0x41;           // type: Type A
      header[2] = 0xFF;           // router
      header[3] = 0xAA;           // source
      header[4] = 0xFF;           // recipient
      header[5] = 0xAA;           // sender
      header[6] = 0x0E;           // ttl
    ...
    // Forge a Route Request to Receiver Node 
    // and expect the routed payload to be received at the Transmitter Node (0xAA)
    // while sending an ACK back. The Receiver Node should therefore 
    // also be receiving an ACK.
  // Receiver    - Expect: 1 
  // Transmitter - Expect: 1 
    case 22: 
    header[0] = 0x08;           // sizeHeader
      header[1] = 0x44;           // type: Type D Route Request
      header[2] = 0x22;           // router: Receiver Node
      header[3] = 0x55;           // source: Node 5
      header[4] = 0xAA;           // recipient: Parent
      header[5] = 0x55;           // sender: Node 5
      header[6] = 0x0E;           // ttl
    ...
    LoRa.beginPacket();
    LoRa.write(header, 8);
    for(int j=0; j<24; j++){    // 24 byte Payload  
        LoRa.write(0xBB);
    }
      LoRa.endPacket(true);  
      Serial.println(i);   
  ...
  // Same as above but payload is too large   
  // Receiver    - Expect: 0 (doesn't go past ldm_checkFrameHeader)
    case 23: 
      header[0] = 0x08;           // sizeHeader
      header[1] = 0x44;           // type: Type A
      header[2] = 0x22;           // router: Receiver Node
      header[3] = 0x55;           // source: Node 5
      header[4] = 0xAA;           // recipient: Parent
      header[5] = 0x55;           // sender: Node 5
    ...
    LoRa.beginPacket();
    LoRa.write(header, 8);
    for(int j=0; j<25; j++){    // 25 byte Payload  
        LoRa.write(0xBB);
    }
      LoRa.endPacket(true);  
      Serial.println(i);   
