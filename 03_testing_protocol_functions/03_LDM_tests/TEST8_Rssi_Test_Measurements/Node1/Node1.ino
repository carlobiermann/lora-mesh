...
Same Code as TEST7 but with more frequent packet transmissions 
the measure the RSSI values that Node 1 and Node 2 save. 

Parent and Node 1 will be stationary. Node 2 will move away from 
Node 1. All Nodes will contiuously send packet. 

Timing Parameters: 

Periodic Payload transmissions: 30.000 ms 
Periodic Rouitng Status broadcasts: 15.000 ms
Delete Routing Table entries older than: 20.000 ms

Serial.print() for debugging purposes is disabled.
...
void ldm_deleteOldEntries(){
  ...
  // when an entry's time value is older than 20.000 ms.
