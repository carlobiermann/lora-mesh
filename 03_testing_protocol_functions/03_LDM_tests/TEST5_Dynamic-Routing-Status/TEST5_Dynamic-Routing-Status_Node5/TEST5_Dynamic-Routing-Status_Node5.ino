/*
Add each Node in this particular order to the 
network one bye one. Observe their Routng Statuses, 
the exchange of Routing Status Broadcasts and the filling 
...
void loop(){
  ...
  if(result == 1){
      ...
      Serial.print("localNextHopID: 0x");
      Serial.println(localNextHopID, HEX);
  
      Serial.print("localHopCount: 0x0");
...
int ldm_bcastRoutingStatus(int mode){
  ...
  byte type = 0x00;
  int result = ldm_setRoutingStatus();
