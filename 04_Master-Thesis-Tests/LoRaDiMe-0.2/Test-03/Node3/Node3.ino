...
// Timing Parameters
#define RS_BCAST_TIME 120000
#define PL_TX_TIME 240000
#define DELETION_TIME 122000
#define ARQ_TIME 2000

// Link Hysteresis Threshold
#define LINK_THRESHOLD 20
...
byte localNextHopID = 0x00;
byte localHopCount = 0x00;
int maxLinkRssi = -10000000;
int currentLinkRssi = -10000000;
int attemptedPayloadTx = 0;
...
bool ldm_checkIfRsEmpty();
bool ldm_checkBadLink();
int ldm_findNewRS();
...
int ldm_insertRoutingTable(
  ...
  if(validNode && validHop){
    ...
    if (nodeID == localNextHopID){
      // in case BCAST is from current RS then update the Rssi values
      // to compare them later on
      
      currentLinkRssi = Rssi;
      if (currentLinkRssi > maxLinkRssi){
        maxLinkRssi = currentLinkRssi;  
      }
    }
...
bool ldm_checkIfRsEmpty(){
  // checks if the an entry of the localNextHopID 
  // still exists in the Routing Table. Reason 
  // for deletion would be, that no BCAST of 
  // localNextHopID has been received within 
  // the DELETION_TIME period. 
  // If localNextHopID wasn't set yet, then idToIndex
  // returns 0
  
  int index = idToIndex(localNextHopID);
  
  Serial.println("Debug checkIfRsEmpty: ");
  Serial.println("localNextHopID to index: ");
  Serial.println(index);
  Serial.println("routingTable[19*index]: ");
  Serial.println(routingTable[19*index]);
  Serial.println("localNextHopID");
  Serial.println(localNextHopID);
  
  if(localNextHopID != routingTable[19*index] || localNextHopID == 0x00){
    Serial.println("ldm_checkIfRsEmpty returns true");
    return true;  
  }
  Serial.println("ldm_checkIfRsEmpty returns false");
  return false;
}

bool ldm_checkBadLink(){

  if((maxLinkRssi - currentLinkRssi) > LINK_THRESHOLD){
    Serial.println("ldm_checkBadLink returns true");
    return true;
  } 
  Serial.println("ldm_checkBadLink returns false");
  return false;
}
...
bool ldm_checkIfEmpty(){
  ...
  for(int i=0; i<8; i++){
    ...
    if (entry != 0x00){
      Serial.println("table not empty");
      return false; 
    }
  }
...
int ldm_findNewRS(){
  ...
  if(!parentFound){
    ...
    localNextHopID = 0xAA;
    localHopCount = 0x01;
    maxLinkRssi = *(int *) (&routingTable[3]);
    return 1;
  }
}
...
int ldm_setRoutingStatus(){
  ...
  bool tableEmpty = ldm_checkIfEmpty();
  if(!tableEmpty){
    bool rsEmpty = ldm_checkIfRsEmpty();
    // Serial.println(currentLinkRssi);
    // Serial.println(maxLinkRssi);
    bool badLink = ldm_checkBadLink();

    // Serial.println("before rsEmpty");
    if (rsEmpty || badLink){
      // Serial.println("passed the check");
      int res = ldm_findNewRS();
      Serial.println("RS set");
      return res;
    } else {
      // Routing Status doesn't need to be reset
      Serial.println("RS didn't need to be reset");
      return 1; 
    }
  } else {
    // Routin Table is empty
    localNextHopID = 0x00;
    localHopCount = 0x00;
    return -1;
  }
  ...
  if (mode == 0){               // Node Mode
    switch(header[1]){            // check type
      ...
      case 0x43:              // Type C: Direct PL
        attemptedPayloadTx++;
        memcpy(&payload[16], (uint8_t *) &attemptedPayloadTx, sizeof(attemptedPayloadTx));
...
void generatePayload(){
  ...
  int nextHopInt = (int) localNextHopID;
  int linkRssi = maxLinkRssi;

  Serial.print("nextHopInt: ");
  Serial.println(nextHopInt);
  Serial.print("maxLinkRssi: ");
